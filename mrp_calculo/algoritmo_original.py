# -*- coding: utf-8 -*-
"""Código original fornecido pelo usuário (com leituras de arquivo desativadas)."""
# -*- coding: utf-8 -*-
"""pacote_mrp.ipynb
...
Original file is located at
    https://colab.research.google.com/drive/1w2i1SSt2KgDWJicgl28wqpQVeWuSU1OD
"""
...
for c in (col_atual, col_destino):
    if c not in df_map.columns:
        raise ValueError(
            f"Coluna '{c}' não encontrada no XLSX. Colunas disponíveis: {list(df_map.columns)}"
        )
m = (
    df_map[[col_atual, col_destino]]
    .dropna()
    .astype(str)
    .apply(lambda s: s.str.strip())
    .drop_duplicates(subset=[col_atual])
)
m = m[m[col_destino] != ""]
dicionario = dict(zip(m[col_atual], m[col_destino]))



faltantes = [k for k in dicionario if k not in df.columns]
if faltantes and False:
    raise KeyError(f"Colunas do mapeamento ausentes no CSV: {faltantes}")
...
# -*- coding: utf-8 -*-
"""MRP_Calculo-dos-Indicadores_Mare.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/17KOqDFzfdBp2GmD8ytVKcWptVfScmbaH
"""
